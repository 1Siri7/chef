...
function editDish(button) {
  ...
  // Get the text inside the li excluding buttons and image
  ...
  if (!textNode) {
    alert("Could not find dish name and price text.");
    return;
  }

  const dishText = textNode.textContent.trim(); // like "seeeseee - ₹55"
  const parts = dishText.split(" - ₹");
  if (parts.length !== 2) {
    alert("Unexpected text format.");
    return;
  }
